"""
The primary function of the current code file is to perform targeted attacks on a specified convolutional neural network model using SPLA based on the infinite norm.
...
# L-infinity epsilon for the attack constraint (e.g., max perturbation per pixel)
eps_inf = 8 / 255.0  # Typical value for ImageNet, adjust as needed

# Iterative step size
iteration_step_size = 0.001

# Calculate the maximum number of iterations (retained for compatibility)
degree = 255
...
for file_name in file_list:
    ...
    atk = PCAIFGSM(model, eps=eps_inf, alpha=0.017, steps=max_num_iterative, mask=mask, targeted=True)
    ...
    # For L-inf minimization: compute delta = adv - orig in pixel space
    ...
    # Binary search for minimal lambda such that orig + lambda * delta is classified as target, with L-inf clip
    iterative_num = 1
    left = 0.0
    right = 1.0
    ...
    while (right - left > epsilon):
        ...
        # Enforce L-inf constraint: clip perturbations to [-eps_inf * 255, eps_inf * 255] in pixel space
        candidate_matrix = np.clip(candidate_matrix, actual_matrix - eps_inf * 255, actual_matrix + eps_inf * 255)
