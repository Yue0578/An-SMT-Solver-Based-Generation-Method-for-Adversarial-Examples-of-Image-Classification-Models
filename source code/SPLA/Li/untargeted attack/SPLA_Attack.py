...
for file_name in file_list:
    ...
    # Use L-infinity constraint in SPLA (assuming SPLA supports eps as L-inf bound)
    atk = SPLA(model, eps=eps_inf, alpha=0.017, steps=max_num_iterative, mask=mask)
    ...
    # Binary search for minimal lambda such that orig + lambda * delta is adversarial, with L-inf clip
    iterative_num = 1
    left = 0.0
    right = 1.0
    ...
    while (right - left > epsilon):
        ...
        if iterative_image_current_top1_label == actual_image_top1_index:
            # Still classified correctly, need larger lambda
            left = mid
        else:
            # Misclassified, can try smaller lambda
            right = mid
